import 'package:flutter/material.dart';

// const Color primaryColor = Color.fromRGBO(230, 46, 4, 1);
// const Color primaryColor = Color.fromRGBO(185, 4, 230, 1);

const Color primaryColor = Color.fromRGBO(19, 9, 158, 1);
const Color darkColor = Colors.black;
const Color textfieldGrey = Color.fromRGBO(209, 209, 209, 1);
const Color fontGrey = Color.fromRGBO(107, 115, 119, 1);
const Color darkFontGrey = Color.fromARGB(220, 0, 0, 0);
const Color whiteColor = Colors.white;
const Color lightGrey = Color.fromRGBO(239, 239, 239, 1);
const Color golden = Color.fromRGBO(255, 168, 0, 1);
const Color goldenlight = Color(0xfffEEAD1);
const Color decColor = Colors.blueAccent;


// const Color darkFontGrey = Color.fromRGBO(62, 68, 71, 1);
// const Color whiteColor = Color.fromRGBO(255, 255, 255, 1);